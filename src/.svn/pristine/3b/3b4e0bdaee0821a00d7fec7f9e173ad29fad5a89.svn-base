...
public static Pair<Map<Entity, Double>, Map<Entity, Double>> execute(Configuration configuration, Query query) throws Exception
{
	...
	println("New Community  in case of category based filtering" + StringUtilsQueryExpansion.MapDoubleValueToString(filteredCommunity));
...
public static Set<Map<Entity, Double>> selectBestCommunities(Configuration configuration, Set<Map<Entity, Double>> comSimilarityBased, Set<String> tokens) throws Exception
{
	...
	if (configuration.DEBUG_INFO)
	{
		...
		{
			println("===============");
			println("Community " + i + ": " + m.getValue() + ":" + StringUtilsQueryExpansion.MapDoubleValueToString(m.getKey()));
			println("===============");
			i++;
		}
	}
